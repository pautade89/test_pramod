...
def business_rule_cnb_df() -> pd.DataFrame:
    ...
    # Interpreted from images: choose non-struck values when two values are shown.
    rules = [
        (102, "60%"), (104, "50%"), (106, "45%"), (107, "75%"), (110, "10%"), (111, "15%"),
        (112, "40%"), (120, "15%"), (121, "35%"), (124, "20%"), (127, "55%"), (135, "40%"),
        (143, "20%"), (144, "25%"), (145, "35%"), (146, "40%"), (147, "41%"), (148, "57%"),
        (155, "35%"), (160, "10%"), (164, "15%"), (165, "30%"), (166, "40%"), (170, "45%"),
    ...
    seen = set()
    cleaned = []
    for seg, rate in rules:
        if seg in seen:
            continue
        seen.add(seg)
        cleaned.append((seg, rate))

    data = []
    for i, (seg, rate) in enumerate(cleaned, start=1):
        data.append([i, seg, rate])
    return pd.DataFrame(data, columns=["Sr No", "Segment ID", "LGD Rate"])
...
def apply_pivot_business_rule_validation(
    ...
    """
    Business Rule validation for pivot tables.

    Coloring is applied on LGD (Average) cells only:
      - Business rule mismatch => light red
      - If rule matches, preserve run-to-run meaning:
            After vs Before same => light green
            After vs Before different => light yellow
...
def build_validation_excel(
    ...
    # Run-to-run compare (green/yellow)
...
def main():
    ...
    if can_generate(CNB_OUTER_PREFIX, "CNB_Validation.xlsx"):
        ...
        build_validation_excel(
            ...
            rule_checks=[{
                'table_index': 2,      # facility pivot index
