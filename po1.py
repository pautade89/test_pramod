...
def find_by_prefix(folder: Path, prefix: str, suffixes=TAR_SUFFIXES) -> Path:
    """
    Find a file directly under 'folder' whose name starts with prefix and ends with one of suffixes.
...
def find_inside_extracted(root: Path, prefix: str, suffixes=TAR_SUFFIXES) -> Path:
    """
    Search recursively under root for a file that starts with prefix and ends with suffixes.
    """
    matches = []
    for p in root.rglob("*"):
        if p.is_file() and p.name.startswith(prefix) and p.name.endswith(suffixes):
...
def find_psv_by_glob(root: Path, pattern: str) -> Path:
    ...
    If multiple, pick newest by modified time.
    """
...
def read_psv_to_df(psv_path: Path) -> pd.DataFrame:
    # Use '|' as delimiter; preserve content
    return pd.read_csv(
        psv_path,
        sep="|",
        dtype=str,
...
def load_psv_dfs_from_run(run_folder: Path, outer_prefix: str, inner_prefixes, psv_patterns):
    ...
    - extract it
    - find each inner tar by prefix within extracted outer
    - extract all inner tars
    - read requested PSV patterns from combined extracted inner content
    """
