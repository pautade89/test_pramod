...
# =========================
# Main (Context 8 generation rules)
# =========================
def main():
    # Generate based ONLY on After_Run presence
    has_cnb = tar_present_by_prefix(AFTER_DIR, CNB_OUTER_PREFIX)
    has_ccms = tar_present_by_prefix(AFTER_DIR, CCMS_OUTER_PREFIX)
    has_comm = tar_present_by_prefix(AFTER_DIR, COMM_OUTER_PREFIX)
    ...
    if has_cnb:
        ...
        print(f"❌ {CNB_OUTER_PREFIX}_*.tar tar is absent in After_Run Folder so CNB_Validation.xlsx excel file is not generated.")
    ...
    if has_ccms:
        ...
        print(f"❌ {CCMS_OUTER_PREFIX}_*.tar tar is absent in After_Run Folder so CCMS_Validation.xlsx excel file is not generated.")
    ...
    if has_comm:
        ...
        print(f"❌ {COMM_OUTER_PREFIX}_*.tar tar is absent in After_Run Folder so CMS_Validation.xlsx excel file is not generated.")
